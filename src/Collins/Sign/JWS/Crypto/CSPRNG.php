<?php
// For licence... see https://github.com/cubiclesoft/php-csprng
namespace Collins\Sign\JWS\Crypto;
...
class CSPRNG
{
    ...
    public function __construct($cryptosafe = false)
    {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === "WIN") {
            ...
            if ($this->mode === false && PHP_VERSION_ID > 50300 && function_exists(
                    "mcrypt_create_iv"
                )
            ) {
                $this->mode = "mcrypt";
        ...
        do {
            switch ($this->mode) {
                case "openssl":
                    ...
                    break;
                case "mcrypt":
                    $data = @mcrypt_create_iv($length, ($this->cryptosafe ? MCRYPT_DEV_RANDOM : MCRYPT_DEV_URANDOM));
                    break;
                case "file":
    ...
    protected static function RNG_Translate()
    {
        ...
        return call_user_func_array(
            (defined("CS_TRANSLATE_FUNC") && function_exists(CS_TRANSLATE_FUNC) ? CS_TRANSLATE_FUNC : "sprintf"),
            $args
        );
    }
}
